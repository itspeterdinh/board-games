...
export const WW_ROLES = {
  WEREWOLF: {
    ...
    desc: 'Each night, choose a villager to eliminate. By day, blend in and avoid suspicion.',
  },
  WHITE_WOLF: {
    name: 'White Wolf',
    team: 'werewolves',
    ...
    desc: 'You are a werewolf, but the Seer sees you as safe. Hunt with the pack each night.',
  },
  VILLAGER: {
    name: 'Villager',
    team: 'village',
    ...
    desc: 'No special power. Use logic and persuasion to find and eliminate the werewolves.',
  },
  SEER: {
    name: 'Seer',
    team: 'village',
    ...
    desc: 'Each night, investigate one player — you learn if they are a Werewolf or not.',
  },
  DOCTOR: {
    name: 'Doctor',
    team: 'village',
    ...
    desc: 'Each night, protect one player from the wolves. You cannot protect the same person two nights in a row.',
  },
  HUNTER: {
    name: 'Hunter',
    team: 'village',
    ...
    desc: 'If you are eliminated (day or night), you immediately take one player down with you.',
  },
  WITCH: {
    name: 'Witch',
    team: 'village',
    ...
    desc: 'You have one save potion and one poison potion for the whole game. Use them wisely.',
  },
  CUPID: {
    name: 'Cupid',
    team: 'village',
    icon: '💘',
    color: '#e91e8c',
    bg: 'linear-gradient(145deg,#2a0018,#3d0026)',
    border: '#e91e8c',
    glow: 'rgba(233,30,140,0.4)',
    desc: 'On the first night, choose two players to be lovers. If one dies, the other dies of heartbreak.',
  },
};
...
export function maxWolves(playerCount) {
...
export function assignWerewolfRoles(
  ...
  const totalWolves =
    wolfCount ?? WOLF_COUNT[count] ?? Math.max(1, Math.floor(count / 4));
...
export function buildWerewolfNightInfo(players, roles) {
  ...
  const wolfNames = players
    .filter((p) => WOLF_ROLES.includes(roles[p.id]))
    .map((p) => p.displayName);
  players.forEach((p) => {
    ...
    info[p.id] = isWolf
      ? {
          sees: allies,
          label: allies.length ? 'Your wolf allies:' : 'You are the lone wolf.',
        }
      : { sees: [], label: '' };
  });
  return info;
...
export function getInitialNightPhase(roles, alivePlayers, round) {
  if (round === 1 && alivePlayers.some((uid) => roles[uid] === 'CUPID'))
    return 'cupid';
  return 'wolves';
}
...
export function getNextNightPhase(
  ...
  // Special roles are called even if dead so host can fake-call them (host force-advances)
  const hasAny = (r) => Object.values(roles || {}).some((v) => v === r);
  const witchExists = hasAny('WITCH');
  const witchActive = witchExists && (!witchUsedSave || !witchUsedPoison);
      ...
      return hasAny('DOCTOR')
        ...
        : hasAny('HUNTER')
          ...
          : witchActive
            ...
            : 'resolve';
    case 'doctor':
      return hasAny('HUNTER') ? 'hunter' : witchActive ? 'witch' : 'resolve';
    case 'hunter':
      return witchActive ? 'witch' : 'resolve';
    case 'witch':
      return 'resolve';
    default:
      return 'resolve';
...
export function resolveNightDeaths({
  ...
  if (wolfKillTarget) {
    const saved =
      nightDoctorTarget === wolfKillTarget || nightWitchSave === wolfKillTarget;
    if (!saved) deaths.push({ id: wolfKillTarget, killedBy: 'wolves' });
  }
  // Witch poison (doctor can block if setting enabled)
  if (nightWitchPoison && !dead(nightWitchPoison)) {
    const blockedByDoctor =
      doctorBlocksPoison && nightDoctorTarget === nightWitchPoison;
    if (!blockedByDoctor)
  ...
  if (
    hunterDied &&
    hunterNightTarget &&
    alivePlayers.includes(hunterNightTarget) &&
    !dead(hunterNightTarget)
  ) {
...
export function checkWerewolfWin(alivePlayers, roles) {
  const wolves = alivePlayers.filter((id) =>
    WOLF_ROLES.includes(roles[id]),
  ).length;
  const others = alivePlayers.filter(
    (id) => !WOLF_ROLES.includes(roles[id]),
  ).length;
