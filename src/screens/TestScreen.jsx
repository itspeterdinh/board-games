...
const FAKE_NAMES = [
  ...
  'Kevin', 'Luna', 'Mike', 'Nina',
]

function fakeUser(name, i) {
  return { uid: `test_${name.toLowerCase()}_${i}`, displayName: name, photoURL: null }
}

// ── Setup form ────────────────────────────────────────────────────────────────

function SetupForm({ user, onCreated }) {
  ...
  const totalLabel  = gameType === 'werewolf'
  ...
  async function create() {
    ...
    try {
      ...
      const fakes = FAKE_NAMES.slice(0, fakeCount).map((n, i) => fakeUser(n, i))
  ...
  return (
    <div className="screen">
      <div className="header-row">
        <div className="screen-title" style={{ flex: 1, textAlign: 'center' }}>🧪 Test Mode</div>
      </div>
      <div style={{ fontSize: '0.85rem', color: 'var(--muted)', textAlign: 'center' }}>
        Creates a room with fake players so you can switch between every perspective.
      </div>
      ...
      <div className="field">
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          {[{ key: 'avalon', icon: '⚔️', name: 'Avalon' }, { key: 'werewolf', icon: '🐺', name: 'Werewolf' }].map(g => (
            <div key={g.key} onClick={() => { setGameType(g.key); setPlayerCount(6) }} style={{
              ...
              boxShadow: gameType === g.key ? '0 0 12px rgba(201,168,76,0.2)' : 'none',
            }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
          {Array.from({ length: maxPlayers - minPlayers + 1 }, (_, i) => i + minPlayers).map(n => (
            <button key={n} onClick={() => setPlayerCount(n)} style={{
              ...
              borderColor: playerCount === n ? 'var(--gold)' : 'var(--border)',
              background: playerCount === n ? 'var(--gold)' : 'var(--surface2)',
...
function RoomView({ user, fakePlayers, roomId, gameType, onReset }) {
  ...
  useEffect(() => {
    ...
    if (room.status === 'ww_night' && room.nightPhase) {
      ...
      switch (phase) {
        case 'wolves': {
          const wolves = fakePlayers.filter(p => room.roles[p.uid] === 'WEREWOLF' && alive(p.uid))
          if (wolves.length) target = wolves[Math.floor(Math.random() * wolves.length)]
          break
        }
        case 'seer':   target = fakePlayers.find(p => room.roles[p.uid] === 'SEER'   && alive(p.uid)); break
        case 'doctor': target = fakePlayers.find(p => room.roles[p.uid] === 'DOCTOR' && alive(p.uid)); break
        case 'hunter': target = fakePlayers.find(p => room.roles[p.uid] === 'HUNTER' && alive(p.uid)); break
        case 'witch':  target = fakePlayers.find(p => room.roles[p.uid] === 'WITCH'  && alive(p.uid)); break
        case 'cupid':  target = fakePlayers.find(p => room.roles[p.uid] === 'CUPID'  && alive(p.uid)); break
        default: break
      }
  ...
  const tabs = [
    ...
    ...fakePlayers.map((p, i) => ({ uid: p.uid, label: p.displayName })),
  ]

  async function handleLeave() {
    // Clean up the room on exit
    try { await leaveRoom(user, roomId) } catch {}
    onReset()
  }
  ...
  if (loading) {
    return (
      <div style={{ paddingTop: 48 }}>
        <SwitcherBar tabs={tabs} activeIdx={activeIdx} onSwitch={setActiveIdx} onReset={handleLeave} />
        <div className="screen"><div className="spinner" style={{ margin: '60px auto' }} /></div>
      </div>
    )
  }
  ...
  if (!room) {
    return (
      <div style={{ paddingTop: 48 }}>
        <SwitcherBar tabs={tabs} activeIdx={activeIdx} onSwitch={setActiveIdx} onReset={onReset} />
        <div className="screen">
          <div className="card text-center">
            <div className="text-muted">Room was deleted or not found.</div>
            <button className="btn btn-ghost btn-small" onClick={onReset} style={{ marginTop: 10 }}>Reset</button>
          </div>
        </div>
      </div>
    )
  ...
  return (
    <div style={{ paddingTop: 48 }}>
      <SwitcherBar tabs={tabs} activeIdx={activeIdx} onSwitch={setActiveIdx} onReset={handleLeave} />
      {room.status === 'lobby'
        ? <LobbyScreen key={activeUser.uid} user={activeUser} room={room} onLeave={handleLeave} />
        : <GameScreen  key={activeUser.uid} user={activeUser} room={room} onLeave={handleLeave} />
      }
    </div>
  )
}
...
function SwitcherBar({ tabs, activeIdx, onSwitch, onReset }) {
  return (
    <div style={{
      ...
      background: 'rgba(13,13,26,0.97)', borderBottom: '1px solid var(--border)',
      ...
      {tabs.map((t, i) => {
        ...
        return (
          <button key={t.uid} onClick={() => onSwitch(idx)} style={{
            flexShrink: 0,
...
export default function TestScreen({ user, onBack }) {
  ...
  if (!session) {
    return <SetupForm user={user} onCreated={setSession} />
  }
