...
import {
  assignWerewolfRoles, buildWerewolfNightInfo, WOLF_ROLES,
  getInitialNightPhase, getNextNightPhase,
  resolveNightDeaths, checkWerewolfWin,
  WW_ROLES,
} from './werewolf'
...
async function saveWerewolfHistory(room, winner) {
  ...
  const realPlayers = room.players.filter(p => !p.isBot && p.id !== room.hostId)
  for (const player of realPlayers) {
    ...
    await setDoc(histRef, {
      ...
      players: room.players.map(p => ({ displayName: p.displayName, role: room.roles?.[p.id] || null })),
...
export async function startWerewolfGame(roomId, optionalRoles = [], wolfCount = null) {
  ...
  if (gamePlayers.length < 5) throw new Error('Need at least 5 players (not counting the host/moderator)')
...
async function advanceNightPhase(roomId, extraUpdate = {}) {
  ...
  const { roles, alivePlayers, nightPhase, witchUsedSave, witchUsedPoison } = room

  const next = getNextNightPhase(roles, alivePlayers, nightPhase, witchUsedSave, witchUsedPoison)
  ...
  if (next === 'resolve') {
    ...
    const newDead = [...(room.deadPlayers || []), ...deaths.map(d => ({
      id: d.id,
    ...
    const update = {
      ...
      status: winner ? 'ended' : 'ww_dayReveal',
      winner: winner || null,
    }
...
export async function confirmWolfKill(roomId, userId) {
  ...
  const aliveWolves = (room.alivePlayers || []).filter(id => WOLF_ROLES.includes(room.roles[id]))
  const allConfirmed = aliveWolves.length > 0 && aliveWolves.every(id => wolfConfirmed[id])

  if (allConfirmed) {
    // Determine winning target (majority, no ties — UI already enforces this)
    const tally = {}
    Object.values(room.wolfVotes || {}).forEach(t => { tally[t] = (tally[t] || 0) + 1 })
    const max = Math.max(...Object.values(tally))
    const target = Object.keys(tally).find(k => tally[k] === max)
    await advanceNightPhase(roomId, { wolfVotes: room.wolfVotes, wolfConfirmed, wolfKillTarget: target })
  } else {
...
export async function submitDoctorTarget(roomId, userId, targetId) {
  const snap = await getDoc(roomRef(roomId))
  const room = snap.data()
...
export async function submitWitchAction(roomId, userId, { save, poison }) {
  const snap = await getDoc(roomRef(roomId))
  const room = snap.data()
  const update = {}
  if (save !== undefined) { update.nightWitchSave = save; update.witchUsedSave = true }
  if (poison !== undefined) { update.nightWitchPoison = poison; update.witchUsedPoison = true }
  await advanceNightPhase(roomId, update)
}

export async function skipNightAction(roomId, userId) {
...
export async function advanceFromDayReveal(roomId) {
  // Hunter auto-fires during night resolution now — go straight to day discussion
  await updateDoc(roomRef(roomId), { status: 'ww_day', dayVotes: {}, dayEliminated: null })
}
...
export async function castDayVote(roomId, userId, targetId) {
  ...
  if (Object.keys(dayVotes).length >= room.alivePlayers.length) {
    ...
    if (eliminated) {
      ...
      let finalAlive = newAlive.filter(id => !extraDeaths.map(d => d.id).includes(id))

      const playerMap = Object.fromEntries(room.players.map(p => [p.id, p]))
      let newDead = [
        ...(room.deadPlayers || []),
        { id: eliminated, displayName: playerMap[eliminated]?.displayName, role: room.roles[eliminated], killedBy: 'vote', round: room.round },
        ...extraDeaths.map(d => ({ id: d.id, displayName: playerMap[d.id]?.displayName, role: room.roles[d.id], killedBy: d.killedBy, round: room.round })),
      ]

      // Hunter auto-revenge: if the hunter was eliminated, their pre-selected target also dies
      const hunterElim = allEliminated.find(id => room.roles[id] === 'HUNTER')
      if (hunterElim && room.hunterNightTarget) {
        ...
        if (targetFinalAlive) {
          ...
          newDead.push({ id: target, displayName: pm[target]?.displayName, role: room.roles[target], killedBy: 'hunter', round: room.round })
          ...
          if (room.lovers?.includes(target)) {
            ...
            if (other && finalAlive.includes(other)) {
              ...
              newDead.push({ id: other, displayName: pm[other]?.displayName, role: room.roles[other], killedBy: 'heartbreak', round: room.round })
            }
          }
        }
      }
...
export async function advanceFromDayResult(roomId) {
  ...
  const nightPhase = getInitialNightPhase(room.roles, room.alivePlayers, nextRound)
  await updateDoc(roomRef(roomId), {
    status: 'ww_night',
    round: nextRound,
    nightPhase,
...
export async function submitHunterShot(roomId, hunterId, targetId) {
  ...
  const newDead = [
    ...(room.deadPlayers || []),
    { id: targetId, displayName: playerMap[targetId]?.displayName, role: room.roles[targetId], killedBy: 'hunter', round: room.round },
  ]
  ...
  const update = {
    ...
    hunterPendingShot: null,
    status: winner ? 'ended' : room.nightDeaths?.length > 0 ? 'ww_day' : 'ww_night',
    winner: winner || null,
  }

  if (!winner) {
    // If hunter died at night, proceed to day. If died during day result, proceed to next night.
    const fromNight = room.status === 'ww_hunterShot' && (room.dayEliminated == null)
    if (!fromNight) {
      const nextRound = room.round + 1
